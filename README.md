...
| `bug-create` | Creates a well-formatted bug task in `<task-tracker>` after `bug-dig` confirms a real bug. Covers required sections, field values, formatting rules, and the writing style your team expects. Always shows draft for approval first; never creates the task without explicit user confirmation. |
| `bug-comment` | Posts a short follow-up comment on an existing `<task-tracker>` bug task — fresh numbers and what changed, nothing else. Use when re-checking an open ticket after `bug-dig` or during weekly review. Always shows draft first; never posts without explicit confirmation. |
