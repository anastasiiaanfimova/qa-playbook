...
| `task-comment` | Posts a short follow-up comment on an existing `<task-tracker>` bug task — fresh numbers and what changed, nothing else. Use when re-checking an open ticket after `bug-dig` or during weekly review. Always shows draft first; never posts without explicit confirmation. |
